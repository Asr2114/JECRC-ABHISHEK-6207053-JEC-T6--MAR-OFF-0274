from selenium import webdriver
from selenium.webdriver.common.by import By
import time

driver = webdriver.Chrome()

driver.get("https://the-internet.herokuapp.com/")
driver.maximize_window()

time.sleep(2)

checkbox = driver.find_element(By.LINK_TEXT, "Checkboxes")
print("Checkbox link found")

drag = driver.find_element(By.PARTIAL_LINK_TEXT, "Drag")
print("Drag and Drop Link Found")

items = driver.find_elements(By.TAG_NAME, "li")
print("Total <li> elements: ", len(items))

driver.get("https://the-internet.herokuapp.com/tables")
time.sleep(2)

website = driver.find_element(
    By.XPATH,
    "//td[text()='jdoe@hotmail.com']/following-sibling::td[1]"
)
print("Website:", website.text)

# Delete link for Bach
delete = driver.find_element(
    By.XPATH,
    "//td[text()='Bach']/following-sibling::td/a[text()='delete']"
)
print("Delete link found for Bach")

# second table
table2 = driver.find_element(By.XPATH, "(//table)[2]")
print("Second table found")

# $100 row parent
row = driver.find_element(
    By.XPATH,
    "//table[2]//td[text()='$100.00']/ancestor::tr"
)
print("Row for $100.00 found")

driver.quit()